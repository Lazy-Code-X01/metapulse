...
export default function DashboardHome() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700">
      ...
      <div className={`sticky top-0 z-30 -mt-10 transition-all duration-300 -mx-6 md:-mx-10 px-6 md:px-10 ${
        isScrolled 
          ? 'bg-slate-50/80 backdrop-blur-md border-b border-slate-200/50 shadow-sm pt-4 pb-3' 
          : 'bg-transparent pt-8 pb-4'
      }`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className={`space-y-1 transition-all duration-300 ${isScrolled ? 'scale-90 origin-left' : 'scale-100'}`}>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none">
              {greeting}, <span className="text-emerald-600 uppercase text-2xl tracking-tighter">{companyName}</span>
            </h1>
            <p className="text-slate-400 font-medium text-sm">Everything looks good today.</p>
          </div>
          
          <div className={`flex items-center gap-4 bg-white px-5 py-3 rounded-2xl border border-slate-200/60 shadow-sm self-start md:self-center transition-all duration-500 ${
            ...
            <div className="flex flex-col">
              <span className="text-xs font-black text-slate-900 leading-none tracking-tight">{companyName}</span>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Admin</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total data tables', value: stats.totalTables, icon: 'lucide:database', color: 'text-blue-600', desc: 'All the data we track' },
          { label: 'People in charge', value: stats.uniqueOwners, icon: 'lucide:users', color: 'text-purple-600', desc: 'Verified team members' },
          { label: 'Things to fix', value: stats.activeIncidents, icon: 'lucide:flame', color: 'text-red-600', desc: 'Active data problems' },
          { label: 'Quality score', value: `${stats.healthScore}%`, icon: 'lucide:shield-check', color: 'text-emerald-600', desc: 'How healthy your data is' },
        ].map((kpi) => (
          <div key={kpi.label} className="relative overflow-hidden bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm flex flex-col transition-all hover:shadow-md hover:border-slate-300 group">
            {/* Background Icon Blur Effect */}
            <div className="absolute -top-4 -right-4 transition-transform group-hover:scale-125 duration-500">
              <Icon icon={kpi.icon} className={`w-28 h-28 ${kpi.color} opacity-[0.03] blur-[1px]`} />
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
        ...
        <div className="lg:col-span-6 bg-white border border-slate-200/60 rounded-3xl p-8 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h3 className="text-xl font-black text-slate-900 tracking-tight">Data health history</h3>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">The last 7 days</p>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200/60 rounded-3xl p-8 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-xl font-black text-slate-900 tracking-tight leading-none">Your data</h3>
            <span className="bg-slate-900 text-white text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest">{assets.length} Active</span>
          </div>

          <div className="space-y-3 flex-1">
            {assets.slice(0, 6).map((asset) => (
              <div key={asset.id} className="flex items-center justify-between group p-3 hover:bg-slate-50 border border-transparent hover:border-slate-100 rounded-2xl transition-all cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-black text-slate-800 group-hover:text-slate-900 transition-colors uppercase tracking-tight">{asset.name}</p>
                ...
                <div className="flex items-center gap-3">
                   <Icon icon="lucide:chevron-right" width="14" className="text-slate-200 group-hover:text-slate-400 transition-colors" />
                </div>
              </div>
            ))}
      ...
      <div className="bg-white border border-slate-200/60 rounded-3xl p-8 shadow-sm">
        <div className="flex items-center justify-between mb-8">
           <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900 tracking-tight">What happened lately</h3>
        ...
        <div className="space-y-3">
          {incidents.slice(0, 3).map((incident) => (
            <div key={incident.id} className="flex items-center gap-6 p-4 rounded-2xl border border-slate-100 hover:border-emerald-100 hover:shadow-sm transition-all group cursor-pointer" onClick={() => router.push('/dashboard/incidents')}>
              <div className="flex-shrink-0">
                <div className={`w-3 h-3 rounded-full shadow-sm ${
                  incident.status.toLowerCase() === 'open' ? 'bg-red-500 shadow-red-500/20' : 
                  incident.status.toLowerCase() === 'investigating' ? 'bg-amber-400 shadow-amber-400/20' : 'bg-emerald-500 shadow-emerald-500/20'
                }`} />
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-2 mb-1">
                  <span className="text-sm font-bold text-slate-800 uppercase tracking-wide truncate">{incident.assetName}</span>
                  <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">#{incident.id.substring(0, 8)}</span>
                </div>
                <p className="text-xs text-slate-500 truncate italic">&quot;{incident.failureReason}&quot;</p>
              </div>

              <div className="text-right hidden sm:block">
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Status</p>
                 <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border uppercase tracking-widest ${
                    incident.status.toLowerCase() === 'open' ? 'bg-red-50 text-red-700 border-red-100' :
                    incident.status.toLowerCase() === 'investigating' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'
                 }`}>
                   {incident.status}
                 </span>
              </div>
          ...
          {incidents.length === 0 && (
            <div className="py-12 flex flex-col items-center justify-center opacity-40 grayscale italic">
               <p className="text-sm font-medium text-slate-400 tracking-tight">Everything is healthy right now.</p>
            </div>
          )}
        </div>
      </div>
